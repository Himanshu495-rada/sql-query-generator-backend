    ...
    for (const column of table.columns) {
      if (column.isForeignKey) {
        ...
        for (const potentialParentTable of schema.tables) {
          ...
          const potentialPrimaryKey = potentialParentTable.columns.find(col => col.isPrimaryKey);
          ...
          if (potentialPrimaryKey) {
            ...
            if (
              ...
              columnNameLower === `${tableLower}${primaryKeyLower.charAt(0).toUpperCase() + primaryKeyLower.slice(1)}`
            ) {
              relationships.push({
 ...
 * Generate SQL from GUI builder configuration
 */
const generateSqlFromConfig = (config: GuiBuilderConfig, databaseType: string): string => {
  ...
  if (filters && filters.length > 0) {
    whereClause = 'WHERE ' + filters.map(filter => {
  ...
  if (sort && sort.length > 0) {
    orderByClause = 'ORDER BY ' + sort.map(s => {
 ...
 */
const generateSampleQueries = (tableName: string, databaseType: string): { name: string; description: string; sql: string }[] => {
  return [
    {
      name: 'Select All',
