...
import { getQueryType, extractAffectedTables, isSandboxTableExpired } from '../utils/queryUtils';
import { DatabaseConfig, QueryResult, SandboxTableMeta } from '../utils/types';

// Main hybrid query execution function
export async function executeHybridQuery({ userId, connection, sqlQuery }: {
  userId: string;
  ...
  const ttlMinutes = (settings && 'sandboxTtlMinutes' in settings && settings.sandboxTtlMinutes != null)
    ? settings.sandboxTtlMinutes : 60;
  ...
  if (queryType === 'DML') {
    ...
    let sandbox = await prisma.sandboxDb.findUnique({ where: { connectionId: connection.id } });
    if (!sandbox) {
    ...
    for (const table of affectedTables) {
      const meta = await getSandboxTableMeta(sandboxConnId, table, connection.type);
      if (!meta || isSandboxTableExpired(meta, Number(ttlMinutes))) {
        // Copy schema and data for this table from source to sandbox
        await copyTableToSandbox(connection, sandboxConnId, table, connection.type);
...
async function ensureSandboxMetaTable(sandboxConnId: string, dbType: string) {
  ...
  if (dbType === 'POSTGRESQL') {
    ...
    query = 'CREATE TABLE IF NOT EXISTS _sandbox_meta (table_name VARCHAR(255) PRIMARY KEY, last_copied DATETIME)';
  } else if (dbType === 'SQLITE') {
    query = 'CREATE TABLE IF NOT EXISTS _sandbox_meta (table_name TEXT PRIMARY KEY, last_copied TEXT)';
  }
  if (query) {
...
async function getSandboxTableMeta(sandboxConnId: string, table: string, dbType: string): Promise<SandboxTableMeta | null> {
  let query = '';
  ...
  if (result.rows.length > 0) {
    return { tableName: result.rows[0].table_name, lastCopied: new Date(result.rows[0].last_copied) };
  }
  return null;
}

async function updateSandboxTableMeta(sandboxConnId: string, table: string, dbType: string) {
  let query = '';
  if (dbType === 'POSTGRESQL' || dbType === 'SQLITE') {
...
async function copyTableToSandbox(sourceConnection: any, sandboxConnId: string, table: string, dbType: string) {
